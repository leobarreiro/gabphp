...
class MPUsuario extends Persistente {
		...
		* */
		...
		public function pesquisaNovosAmigos($arParam) {
			...
			if (isset($arParam['usuarioSistema'])) {
				$stSql .= " AND codusuario <> " . $arParam['usuarioSistema'] . " ";
			}
			if (isset($arParam['nome'])) {
				$stSql .= " AND nomecompleto LIKE '%" . $arParam['nome'] . "%' ";
			}
			if (isset($arParam['telefone'])) {
				$stSql .= "
				AND
				( telefone LIKE '%" . $arParam['telefone'] . "%' OR
				  celular LIKE '%" . $arParam['celular'] . "%' ) ";
			}
			if (isset($arParam['email'])) {
				$stSql .= " AND email LIKE '%" . $arParam['email'] . "%' ";
			}
			
			$stSql = $this->preparaSQL($stSql);
			$this->stSQL = $stSql;

			$roConsulta = $this->executaSQL($stSql);
			return $roConsulta;
